...
namespace Gateway
{
    public class GatewayMain : ThreadWrapper
    {
        ...
        List<GatewayPlayer> players;
        List<GatewayPlayer> inactivePlayers;

        int nextGateWayPlayerConnectionId = 1024;

        GatewayPlayer currentPlayerForHeader = null;
        ...
        void ManageServerKeepAlive()
        {
            ...
            {
                if (server.IsKeepAliveValid() == false)
                {
#if !DEBUG
                    server.Disconnect();
                    if (gamesToDisconnectPlayers == null)
                    {
                        gamesToDisconnectPlayers = new List<int>();
                    }
                    gamesToDisconnectPlayers.Add(server.gameId);
#endif
                }
            }
            if (gamesToDisconnectPlayers != null)
